...
function renderPanel(g){
  ...
  if (g !== "all") {
    ...
    d.players.forEach(function(p, i) {
      if (p.thumb) h += '<div class="radar-card" onclick="showDetail(' + JSON.stringify(g) + ',' + i + ')"><img src="' + p.thumb + '?v=652120'" alt="' + p.name + '" loading="lazy"><div class="rname">' + p.name + '</div><div class="rteam">' + p.team + '</div></div>';
    });
    h += '</div></div>';
  }
...
function showDetail(g, idx) {
  ...
  if (p.thumb) h += '<img class="modal-radar" src="' + p.thumb + '?v=652120'">';
  if (p.photo) h += '<img class="modal-photo" src="' + p.photo + '?v=652120'">';
