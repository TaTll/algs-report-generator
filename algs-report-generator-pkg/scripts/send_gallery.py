...
draw.text((canvas_w//2 - 150, 15), "ALGS Y6 Split1 Playoffs - Day1 A vs B 选手雷达图", fill=(0, 210, 255), font=font)
...
<title>ALGS Y6 Split1 Playoffs - Day1 选手数据画廊</title>
<style>
...
.card-team { color: #aaa; font-size: 12px; }
...
<body>
<h1>🏆 ALGS Y6 Split1 Playoffs - Day1 A vs B</h1>
<p class="subtitle">''' + str(len(chart_files)) + ''' 名选手 · 点击图片可放大 · 搜索框可筛选</p>
...
for p in player_rows:
    ...
    gallery_html += f'''<div class="card" data-name="{name.lower()}" data-team="{team.lower()}">
  <img src="data:image/png;base64,{b64_img}" alt="{name}" onclick="showDetail(this)">
  <div class="card-body">
    <div class="card-name">{name}</div>
    <div class="card-team">{team} | Group {group} | Best: {bestp}</div>
